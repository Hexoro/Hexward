...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated users can view patient images') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Staff can upload patient images') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Staff can update patient images') THEN
...
-- Add image_url column to patients table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'patients' AND column_name = 'image_url') THEN
    ALTER TABLE public.patients ADD COLUMN image_url text;
  END IF;
END $$;

-- Clear existing demo data to avoid conflicts
DELETE FROM public.patients WHERE name IN ('John Smith', 'Maria Garcia', 'Robert Johnson', 'Lisa Chen', 'David Wilson');
DELETE FROM public.camera_feeds WHERE camera_id LIKE 'CAM-%';
DELETE FROM public.alerts;
DELETE FROM public.ai_detections;
DELETE FROM public.patient_vitals_history;
